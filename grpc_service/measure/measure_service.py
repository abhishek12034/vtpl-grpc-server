...
class MeasureService(BaseService, main_pb2_grpc.MeasureServiceServicer):
    def __init__(self):
    ...
    def MeasureOneD(self, request, context):
        ...
        try:
            ...
            self.job_status[job_id]["object_length"] = round(
                self.processor.calc_ref_val, 2
            )

            self.job_status[job_id]["processed_image_count"] = len(request.in_img_list)

            # Create response
            response = measure_pb2.MeasureResponse(**self.job_status[job_id])

            logger.info(f"Returning response: {response}")
            print(
                self.job_status[job_id]["object_length"],
                self.job_status[job_id],
                response,
            )
            self.store_job_status_in_redis(job_id, self.job_status[job_id])
            ...
            self.job_status[job_id]["status_code"] = JobStatusCode.FAILED.value
            self.job_status[job_id]["error_message"] = str(e)
