...
class adjust_process:
    def __init__(self):
        ...
        self.in_param = ""
        self.p_mat = []
        self.cl_name = ["black", "gray", "white", "red", "green", "blue", "yellow", "cyan", "magenta", "orange", "purple"]
        self.assign_default()
    ...
    def cubic_spline_interpolate_points(self, in_x_list, in_y_list):
        ...
        if (len(in_x_list) == len(in_y_list)) and (len(in_x_list) > 1):
            ...
            for i_cnt in range(len(in_x_list)):
                if in_x_list[i_cnt] >= 0.0 and in_x_list[i_cnt] <= (1.0 * mul_fact) and \
                            in_y_list[i_cnt] >= 0.0 and in_y_list[i_cnt] <= (1.0 * mul_fact):
            ...
            if ini_found_flag:
                for i_cnt in range(base_i_cnt, len(in_x_list) - 1):
                    if in_x_list[i_cnt + 1] >= in_x_list[base_i_cnt] + x_min_diff:
                        if in_x_list[i_cnt + 1] >= 0.0 and in_x_list[i_cnt + 1] <= (1.0 * mul_fact) and \
                                in_y_list[i_cnt + 1] >= 0.0 and in_y_list[i_cnt + 1] <= (1.0 * mul_fact):
        ...
        for i_cnt in range(len(x_list) - 1):
            for k_cnt in range(len(interpolate_pt)):
                mod_x_list.append( ((x_list[i_cnt] * interpolate_pt[k_cnt]) + (x_list[i_cnt + 1] * (1.0 - interpolate_pt[k_cnt]))) )
                mod_y_list.append( ((y_list[i_cnt] * interpolate_pt[k_cnt]) + (y_list[i_cnt + 1] * (1.0 - interpolate_pt[k_cnt]))) )
    ...
    def mod_adjust(self, in_img_path = "",
                            curve_name = "",
                            write_plot_fig_file = "",
        ...
        if process_type == "exposure_control_set_spline":
            ...
            exp_fact = (-10.0 if exposure_times < -10.0 else 10.0 if exposure_times > 10.0 else exposure_times)
            ...
            mod_x, mod_y = self.cubic_spline_interpolate_points(exp_curve_x_list, exp_curve_y_list)
            self.cubic_spline_1 = CubicSpline(mod_x, mod_y)
            
            self.curve_map_np_arr_1 = np.arange(256)
            self.curve_map_np_arr_1 = np.clip(self.cubic_spline_1(self.curve_map_np_arr_1), 0, 255)
        ...
        #curve_set_spline
            ...
            mod_x, mod_y = self.cubic_spline_interpolate_points(curve_x_list, curve_y_list)
            self.cubic_spline_1 = CubicSpline(mod_x, mod_y)
        ...
        #curve_set_spline
            ...
            in_level_st = 0.0 if in_level_st < 0.0 else 1.0 if in_level_st > 1.0 else in_level_st
            in_level_en = 0.0 if in_level_en < 0.0 else 1.0 if in_level_en > 1.0 else in_level_en
            in_level_st = in_level_en if in_level_st > in_level_en else in_level_st
            in_level_mid = in_level_st if in_level_mid < in_level_st else in_level_en if in_level_mid > in_level_en else in_level_mid

            out_level_st = 0.0 if out_level_st < 0.0 else 1.0 if out_level_st > 1.0 else out_level_st
            out_level_en = 0.0 if out_level_en < 0.0 else 1.0 if out_level_en > 1.0 else out_level_en
            ...
            pt_2x = (((in_level_st * mid_seg_2) + (in_level_en * mid_seg_1)) / total_seg) * mul_fact
            ...
            if pt_3x < 255.0:
                level_curve_x_list = np.insert(level_curve_x_list, len(level_curve_x_list), 255.0)
                level_curve_y_list = np.insert(level_curve_y_list, len(level_curve_y_list), 255.0)
            ...
            for k_cnt in range(1, len(mod_x)):                        
                ...
                for i_cnt in range(st_x, en_x):
                    st_line_seg_curve.append(int(np.interp(i_cnt, mod_x, mod_y) + 0.499))
            ...
            for f_i_cnt in range(len(f_path_list)):
                ...
                if np.any(in_img):
                    ...
                    #brightness_change
                        ...
                        brightness_amt_ch = np.clip(brightness_amount_change, -1.0, 1.0) * 255
                    ...
                    if process_type == "brightness_contrast_change":
                        brightness_amt_ch = np.clip(brightness_amount_change, -1.0, 1.0) * 255
                        con_change_fact = np.clip(contrast_change_factor, 0, 255)
                        out_img = cv.convertScaleAbs(in_img, alpha = con_change_fact, beta = brightness_amt_ch)
                    ...
                    #hue_change
                        ...
                        hsv_int_16[..., 0] = np.clip(((hsv_int_16[..., 0] + h_deg_ch) % 180),  0, 255)
                    ...
                    #saturation_change
                        ...
                        hsv_int_16[..., 1] = np.clip((hsv_int_16[..., 1] * s_times_ch), 0, 255)
                        hsv_uint_8 = hsv_int_16.astype(np.uint8)
                    ...
                    #intensity_value_change
                        ...
                        v_amt_ch = np.clip(intensity_value_amount_change, -1.0, 1.0) * 255
                        
                        hsv_int_16[..., 2] = np.clip((hsv_int_16[..., 2] + v_amt_ch), 0, 255)
                        hsv_uint_8 = hsv_int_16.astype(np.uint8)
                    ...
                    #histogram_equalization
                        ...
                        hist, bins = np.histogram(yuv_img[st_row : en_row, st_col : en_col, 0], 256, [0, 256])                        
                        cdf = hist.cumsum()
                        cdf = ((cdf - cdf.min()) * 255 /  max((cdf.max() - cdf.min()), 1)) + 0.499
                    ...
                    #CLAHE
                        ...
                        clahe = cv.createCLAHE(clipLimit = in_clip_limit, tileGridSize = (grid_col, grid_row))
                    ...
                    #contrast_stretch
                        ...
                        st_x = st_x - int(st_x * np.power((1 - in_con_stretch_amt), 4) + 0.4999)
                        en_x = en_x + int(((255 - en_x) * np.power((1 - in_con_stretch_amt), 4)) + 0.4999)
